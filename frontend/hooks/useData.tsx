...
export function useData({ user }: UseDataProps): UseDataReturn {
  ...
  const fetchGraphData = useCallback(
    async (days: number) => {
      ...
      try {
        const responseForeCast = await axios.post(
          `${api}/api/predict/forecast/${user}`,
          {
            numberofdays: days,
          }
        );
        ...
        const responseForeCastByMonth = await axios.post(
          `${api}/api/predict/forecast/by-month/${user}`,
          { numberofdays: 120 }
        );
        ...
        const responseAccuracy = await axios.get(`${api}/api/accuracy/${user}`);
